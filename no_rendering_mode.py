...
class HelpText(object):
    def __init__(self, font, width, height):
        ...
        help_width = min(680, int(width * 0.6))  # 60% of screen width, max 680
        help_height = len(lines) * line_height + 12
        
        self.dim = (help_width, help_height)
        ...
        for n, line in enumerate(lines):
            ...
            self.surface.blit(text_texture, (22, n * line_height))
            self._render = False
...
class HUD (object):
    ...
    def _init_hud_params(self):
        ...
        help_font_size = max(30, int(self.dim[1] * 0.05))  # 5% of screen height, minimum 30
        help_font = pygame.font.Font(mono, help_font_size)
...
class World(object):
    ...
    def _spawn_hero(self):
        ...
        # Always get Dodge Charger blueprint
        all_vehicles = self.world.get_blueprint_library().filter(self.args.filter)
        blueprint = None
        for bp in all_vehicles:
            if "charger" in bp.id.lower():
                blueprint = bp
                print(f"Selected Dodge Charger: {bp.id}")
                break
        
        # Fallback to random vehicle if Charger not found
        if not blueprint:
            print("Warning: Dodge Charger not found, using random vehicle")
    ...
    def update_hud_info(self, clock):
        ...
        if self.hero_actor is not None:
            ...
            hero_mode_text = [
                ...
                'Hero Position:',
                '  X: %10.2f m' % hero_location.x,
                '  Y: %10.2f m' % hero_location.y,
                '  Z: %10.2f m' % hero_location.z,
                'Hero Affected by:',
    ...
    def _split_actors(self):
        ...
        for actor_with_transform in self.actors_with_transforms:
            ...
            elif 'static.prop.atm' in actor.type_id:
                chargers.append(actor_with_transform)

        return (vehicles, traffic_lights, speed_limits, walkers, chargers)
    ...
    def _render_chargers(self, surface, list_chargers, world_to_pixel):
        """Renders chargers as colored squares (blue if compromised, grey otherwise)"""
        # Import registry to check compromise status
        try:
            from vlib import charger_registry
        except ImportError:
            charger_registry = None
        
        for charger in list_chargers:
            actor, transform = charger
            # Get the position of the charger
            location = transform.location
            pixel_pos = world_to_pixel(location)
            
            # Define the size of the square (in pixels)
            square_size = 8
            
            # Create a rectangle centered on the charger position
            rect = pygame.Rect(
                pixel_pos[0] - square_size // 2,
                pixel_pos[1] - square_size // 2,
                square_size,
                square_size
            )
            
            # Determine color based on charger state
            color = COLOR_ALUMINIUM_3  # Default grey for normal chargers
            
            # Check if charger is compromised via registry
            if charger_registry and charger_registry.is_charger_compromised(actor.id):
                color = COLOR_SKY_BLUE_0  # Blue for compromised
            
            # Draw the square
            pygame.draw.rect(surface, color, rect)

    def render_actors(self, surface, vehicles, traffic_lights, speed_limits, walkers, chargers):
        ...
        self._render_chargers(surface, chargers, self.map_image.world_to_pixel)
    ...
    def render(self, display):
        ...
        vehicles, traffic_lights, speed_limits, walkers, chargers = self._split_actors()
        ...
        # TODO(sfx): Figure out a faster way to do this.
